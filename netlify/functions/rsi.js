...
exports.handler = async () => {
  try {
    ...
    const results = await Promise.all(top.map(async t => {
      try {
        const kl = await fetch(`https://api.binance.com/api/v3/klines?symbol=${t.symbol}&interval=1d&limit=60`).then(r => r.json());
        const closes = kl.map(k => parseFloat(k[4]));
        const rsi = calcRSI(closes);
        if (rsi === null) return null;
        return { sym: t.symbol.replace('USDT', ''), rsi, chg: parseFloat(t.priceChangePercent), vol: parseFloat(t.quoteVolume), price: parseFloat(t.lastPrice) };
      } catch { return null; }
    }));
